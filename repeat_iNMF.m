function [result,best_performance] = repeat_iNMF(X,vecN,vecPara,repeat,err_n)
% run ncNMF repeat times on X
%<Input>
...
%        repeat: times of applying iNMF+ on each data matrix with each ran
%        err_n: the number used in select the top err_n minmum error
%        solution
%<Output>
%       result: the repeat times result of iNMF+ 
...
parfor i = 1:repeat
    [W,H,err,elapse] = iNMF(X,vecN,vecPara);
    result{1,i}.W = W;
    result{1,i}.H = H;
    result{1,i}.err=err;
